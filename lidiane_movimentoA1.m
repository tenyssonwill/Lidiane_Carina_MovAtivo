...
f(1) = figure('Name',filename(1:end-4),'visible','on','units','normalized','outerposition',[0 0 1 1]);
f(2) = figure('Name',filename(1:end-4),'visible','on','units','normalized','outerposition',[0 0 1 1]);
f(3) = figure('Name',filename(1:end-4),'visible','on','units','normalized','outerposition',[0 0 1 1]);
f(4) = figure('Name',filename(1:end-4),'visible','on','units','normalized','outerposition',[0 0 1 1]);

window_time = 0.375;
overlap_time = window_time/2;

% Janela em pontos
window_length = floor(window_time*emg_fs);
overlap_length = floor(overlap_time*emg_fs);

for j = 1:4
    figure(f(j))
    for i = 1:2
        subplot(2,2,2*i-1)
        plot(emg_time,emg_filtered(2*(j-1)+i,:),'k')
        title(emg_names{2*(j-1)+i})
        y = get(gca,'YLim');
        ...
        subplot(2,2,2*i)
        NFFT = 2^(nextpow2(window_length)-1);
        window = hanning(NFFT);
        spectrogram(emg_filtered(2*(j-1)+i,:),window,overlap_length,101,emg_fs,'yaxis');
        ylim([0 500])
        title('Spectrogram')%
    end
end
...
dt = 1/emg_fs; 
for i=1:emg_channels
   mcu_env_emg_pre{i} = dt*trapz(abs(filtfilt(b,a,abs(emg_pre{i}))));
   mcu_env_emg_ida{i} = dt*trapz(abs(filtfilt(b,a,abs(emg_ida{i}))));
   mcu_env_emg_vol{i} = dt*trapz(abs(filtfilt(b,a,abs(emg_vol{i}))));
...
% Analise On-Off
resp =  questdlg({'1: Janela = 25 ms e 3*DevPad',...
    '1: Janela = 50 ms e 1*DevPad','2: Janela = 25 ms e 2*DevPad'},'Escolha','1','2','3','1');
switch(resp)
    case '1'
...
for i=1:emg_channels
   ...
   onoff = emgonoff2(emg_filtered(i,:)',emg_fs,wd,sd,baseline,fc,p);
   if(isempty(onoff))
       disp('Sem alteracao em relacao ao baseline')
   else
       figure(f(ceil(i/2)))
       subplot(2,2,3-2*rem(i,2))
       y = get(gca,'YLim');
       ...
       if(~rem(i,2))
            ...
            print('-djpeg','-r300',[pathname filename(1:end-4) ...
                '_' emg_names{i-1}(1:strfind(emg_names{i-1},':')-1) ...
                '_' emg_names{i}(1:strfind(emg_names{i},':')-1) ...
                '_wd' num2str(wd) '_std' num2str(sd) '_EMG'])
            close
       end
   end
end
...
save([pathname filename(1:end-4) '_processados_wd' num2str(wd) '_std' num2str(sd)],'mcu_env_emg_pre','mcu_env_emg_ida',...
...
function onoff = emgonoff2(rawemg, fs, ws, sd,bl,fc,p)
% EMGONOFF - Find on/off times and indicies of raw EMG data.
% Calculate average(mean) value of resting EMG
% Define "on" EMG as the sample where average value of EMG in a given window
% range around the sample is a given # of std. dev. above avg. resting EMG.
...
%bslnON_srt  = 1;
bslnON_srt  = p(1)-fs*200e-3;
bslnOFF_end = p(3)+1*fs;
%bslnOFF_end = 10*fs;
x = round([bslnON_srt bslnON_srt+bl*(fs/1000)-1 bslnOFF_end bslnOFF_end+0.05*fs]);
%x = round([bslnON_srt bslnON_srt+bl*(fs/1000)-1 bslnOFF_end-fs*50e-3 bslnOFF_end]);
%close(f1);clear f1; % Leave commented if you want to keep EMG graph up to 
%                    % do a visual QA of on/off results.
%% preallocate arrays
mvgav = zeros(x(4)-x(1),1);
...
mvgav2 = zeros(x(4)-x(1),1);
i = 0;
xi = round(x(2)); % start OFF search approx. for de ON
for n = 2:length(mvgav2)
    mvgav2(n,1) = mean(fwlo((xi-sws):(xi+sws)));
    if (mvgav2(n) > restav2+sd*reststd2)
        i = i+1;
...
% mvgav2 = zeros(x(4)-x(1),1);
% i = 0;
% xi=onoff(1,1)+1; % start OFF search approx. for de ON
% %% OFF loop:
% xi=round(xi);
% for n=2:length(mvgav2)
%     mvgav2(n,1)=mean(fwlo((xi-sws):(xi+sws)));
